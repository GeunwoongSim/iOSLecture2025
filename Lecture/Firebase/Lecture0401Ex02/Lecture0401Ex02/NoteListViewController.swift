...
class NoteListViewController: UIViewController {
        ...
        Firestore.firestore().collection("users")
            ...
            .addSnapshotListener { [weak self] snapshot, error in
                ...
                self?.notes = documents.map { doc in
                    ...
                    let note = Note(id: doc.documentID, title: title, content: content)
                    return note
                }
    ...
    @objc private func addNote() {
        ...
        let addAction = UIAlertAction(title: "추가", style: .default) { [weak self] _ in
            ...
            // Firestore에 새 노트 추가
            // 컬렉션 -> 도큐멘트 -> 컬렉션 -> 도큐멘트 구조
            Firestore.firestore().collection("users") // users라는 이름의 가장 최상단의 컬렉션
                .document(userId) // users 컬렉션 내부에는 userId의 이름으로 작성된 도큐멘트가 있음
                .collection("notes") // userId의 이름으로 작성된 도큐멘트 내부에 notes라는 내부 컬렉션이 존재
                // notes 컬렉션 내부에 도큐멘트를 추가
